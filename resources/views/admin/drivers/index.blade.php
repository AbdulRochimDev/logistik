@extends('layouts.base', ['title' => 'Driver', 'mainClass' => 'layout-wide'])
...
<style>
    ...
    .status-active { background:rgba(34,197,94,0.2);color:#047857; }
...
<div class="page-shell">
    <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;">
            <h1 style="margin:0;font-size:1.8rem;">Daftar Driver</h1>
            <a href="{{ route('admin.drivers.create') }}" class="btn btn-primary" style="text-decoration:none;">Tambah Driver</a>
        </div>
            ...
            <table class="table">
                ...
                @forelse($drivers as $driver)
                    <tr>
                        ...
                        <td>
                            <span class="status-pill status-{{ $driver->status }}">{{ ucfirst($driver->status) }}</span>
                        </td>
                        <td>{{ $driver->shipments_count }}</td>
                        <td style="text-align:right;display:flex;gap:0.75rem;justify-content:flex-end;">
                            <a href="{{ route('admin.drivers.edit', $driver) }}" class="btn btn-neutral">Edit</a>
                            <form action="{{ route('admin.drivers.destroy', $driver) }}" method="POST" onsubmit="return confirm('Hapus driver ini?');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6">Belum ada driver.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
